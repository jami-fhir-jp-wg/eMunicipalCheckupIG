Profile:        JP_Composition_eMunicipalCheckup_563
Parent:         Composition
Id:             JP-Composition-eMunicipalCheckup-563
//Title:          "自治体検診結果報告書　Compositionリソース　文書構成情報"
Description:    "自治体検診結果報告書　Compositionリソース　文書構成情報(肝炎ウイルス一次と精密検査)"
* ^url = "http://jpfhir.jp/fhir/eMunicipalCheckup/StructureDefinition/JP_Composition_eMunicipalCheckup_563"
* ^status = #draft
...
//  * entry contains OBS 1..* MS and COV 0..2 MS
  * entry contains COV 0..2 MS and
    ...
    9P616000000000011 1..1 MS and
    9P617160800000049 0..1 MS
  ...
  * entry[9P615000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P615000000000011)
    * ^short = "検査結果セクションに含まれる 肝炎ウイルス検診のB型肝炎ウイルス検査判定 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9N401000000000011] only Reference(JP_Observation_eMunicipalCheckup_9N401000000000011)
    * ^short = "検査結果セクションに含まれる 肝炎ウイルス検診のC型肝炎ウイルス検査判定 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P616000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P616000000000011)
    * ^short = "検査結果セクションに含まれる 肝炎ウイルス検診の精密検査結果 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P617160800000049] only Reference(JP_Observation_eMunicipalCheckup_9P617160800000049)
    * ^short = "検査結果セクションに含まれる 肝炎ウイルス検診の精密検査所見 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
...
//  * entry only Reference(JP_Observation_eMunicipalCheckup_9P600000000000011 or JP_Observation_eMunicipalCheckup_9P602000000000011 or JP_Observation_eMunicipalCheckup_9P604000000000011 or JP_Observation_eMunicipalCheckup_9P606000000000011 or JP_Observation_eMunicipalCheckup_9P607000000000011 or JP_Observation_eMunicipalCheckup_9P609000000000011 or JP_Observation_eMunicipalCheckup_9P611000000000011 or JP_Observation_eMunicipalCheckup_9P613000000000011)
  ...
  * entry contains
    ...
    9P613000000000011 0..1 MS
  * entry[9P600000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P600000000000011)
    * ^short = "問診結果セクションに含まれる 肝炎ウイルス検診時の問診：肝臓病歴、肝機能が悪いと言われた経験の有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P602000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P602000000000011)
    * ^short = "問診結果セクションに含まれる 肝炎ウイルス検診時の問診：広範な外科的処置歴の有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P604000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P604000000000011)
    * ^short = "問診結果セクションに含まれる 肝炎ウイルス検診時の問診：妊娠・分娩時の多量出血歴の有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P606000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P606000000000011)
    * ^short = "問診結果セクションに含まれる 肝炎ウイルス検診時の問診：定期的な肝機能検査受診の有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P607000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P607000000000011)
    * ^short = "問診結果セクションに含まれる 肝炎ウイルス検診時の問診：B型肝炎ウイルス検査の受診歴の有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P609000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P609000000000011)
    * ^short = "問診結果セクションに含まれる 肝炎ウイルス検診時の問診：B型肝炎治療歴の有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P611000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P611000000000011)
    * ^short = "問診結果セクションに含まれる 肝炎ウイルス検診時の問診：C型肝炎ウイルス検査の受診歴の有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P613000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P613000000000011)
    * ^short = "問診結果セクションに含まれる 肝炎ウイルス検診時の問診：C型肝炎治療歴の有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
