Profile:        JP_Composition_eMunicipalCheckup_523
Parent:         Composition
Id:             JP-Composition-eMunicipalCheckup-523
//Title:          "自治体検診結果報告書　Compositionリソース　文書構成情報"
Description:    "自治体検診結果報告書　Compositionリソース　文書構成情報(乳がん一次検診と精密検査)"
* ^url = "http://jpfhir.jp/fhir/eMunicipalCheckup/StructureDefinition/JP_Composition_eMunicipalCheckup_523"
* ^status = #draft
...
//  * entry contains OBS 1..* MS and COV 0..2 MS
  ...
  * entry[9P524000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P524000000000011)
    ...
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P525000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P525000000000011)
    * ^short = "検査結果セクションに含まれる 乳がん検診の精密検査結果 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P526160800000049] only Reference(JP_Observation_eMunicipalCheckup_9P526160800000049)
    * ^short = "検査結果セクションに含まれる 乳がん検診の精密検査所見 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P527000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P527000000000011)
    * ^short = "検査結果セクションに含まれる 乳がん検診の精密検査による偶発症の有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
