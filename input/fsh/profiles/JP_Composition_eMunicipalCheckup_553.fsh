Profile:        JP_Composition_eMunicipalCheckup_553
Parent:         Composition
Id:             JP-Composition-eMunicipalCheckup-553
//Title:          "自治体検診結果報告書　Compositionリソース　文書構成情報"
Description:    "自治体検診結果報告書　Compositionリソース　文書構成情報(大腸がん一次検診と精密検査)"
* ^url = "http://jpfhir.jp/fhir/eMunicipalCheckup/StructureDefinition/JP_Composition_eMunicipalCheckup_553"
* ^status = #draft
...
//  * entry contains OBS 1..* MS and COV 0..2 MS
  * entry contains COV 0..2 MS and
    Group_1B040 0..1 MS and
    1B040000001599811 0..1 MS and
    1B040160801599849 0..1 MS and
    9P582000000000011 1..1 MS and
    9P583160800000049 0..1 MS and
    9P584000000000011 0..1 MS and
    9P585000000000011 1..1 MS and
    9P586160800000049 0..1 MS and
    9P587000000000011 0..1 MS
  ...
  * entry[Group_1B040] only Reference(JP_Observation_eMunicipalCheckup_Group_1B040)
    * ^short = "検査結果セクションに含まれる 便潜血検査 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[1B040000001599811] only Reference(JP_Observation_eMunicipalCheckup_1B040000001599811)
    * ^short = "検査結果セクションに含まれる 大腸がん検診の便潜血検査判定 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[1B040160801599849] only Reference(JP_Observation_eMunicipalCheckup_1B040160801599849)
    * ^short = "検査結果セクションに含まれる 大腸がん検診の便潜血検査所見 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P582000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P582000000000011)
    * ^short = "検査結果セクションに含まれる 大腸がん検診の精密検査の対象有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P583160800000049] only Reference(JP_Observation_eMunicipalCheckup_9P583160800000049)
    * ^short = "検査結果セクションに含まれる 大腸がん検診のその他所見 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P584000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P584000000000011)
    * ^short = "検査結果セクションに含まれる 大腸がん検診による偶発症の有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P585000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P585000000000011)
    * ^short = "検査結果セクションに含まれる 大腸がん検診の精密検査結果 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P586160800000049] only Reference(JP_Observation_eMunicipalCheckup_9P586160800000049)
    * ^short = "検査結果セクションに含まれる 大腸がん検診の精密検査所見 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P587000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P587000000000011)
    * ^short = "検査結果セクションに含まれる 大腸がん検診の精密検査による偶発症の有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
...
//  * entry only Reference(JP_Observation_eMunicipalCheckup_9P580000000000011 or JP_Observation_eMunicipalCheckup_9P581000000000011)
  * entry only Reference(JP_Observation_eMunicipalCheckup or JP_Observation_eMunicipalCheckup_Group)
    ...
    * ^slicing.rules = #closed
  * entry contains
    9P580000000000011 0..1 MS and
    9P581000000000011 0..1 MS
  * entry[9P580000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P580000000000011)
    * ^short = "問診結果セクションに含まれる 大腸がん検診の過去の受診歴 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P581000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P581000000000011)
    * ^short = "問診結果セクションに含まれる 大腸がん検診時の大腸がんに係る症状の有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
