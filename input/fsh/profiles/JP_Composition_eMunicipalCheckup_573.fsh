Profile:        JP_Composition_eMunicipalCheckup_573
Parent:         Composition
Id:             JP-Composition-eMunicipalCheckup-573
//Title:          "自治体検診結果報告書　Compositionリソース　文書構成情報"
Description:    "自治体検診結果報告書　Compositionリソース　文書構成情報(骨粗鬆症検診一次と精密検査)"
* ^url = "http://jpfhir.jp/fhir/eMunicipalCheckup/StructureDefinition/JP_Composition_eMunicipalCheckup_573"
* ^status = #draft
...
//  * entry contains OBS 1..* MS and COV 0..2 MS
  ...
  * entry[9P637000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P637000000000011)
    ...
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P638000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P638000000000011)
    * ^short = "検査結果セクションに含まれる 骨粗鬆症検診の精密検査結果 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P639160800000049] only Reference(JP_Observation_eMunicipalCheckup_9P639160800000049)
    * ^short = "検査結果セクションに含まれる 骨粗鬆症検診の精密検査所見 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
