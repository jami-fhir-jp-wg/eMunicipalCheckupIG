Profile:        JP_Composition_eMunicipalCheckup_533
Parent:         Composition
Id:             JP-Composition-eMunicipalCheckup-533
//Title:          "自治体検診結果報告書　Compositionリソース　文書構成情報"
Description:    "自治体検診結果報告書　Compositionリソース　文書構成情報(胃がん一次検診と精密検査)"
* ^url = "http://jpfhir.jp/fhir/eMunicipalCheckup/StructureDefinition/JP_Composition_eMunicipalCheckup_533"
* ^status = #draft
...
//  * entry contains OBS 1..* MS and COV 0..2 MS
  * entry contains COV 0..2 MS and
    ...
    9P549000000000011 1..1 MS and
    9P550160800000049 0..1 MS and
    9P551000000000011 0..1 MS
  ...
  * entry[Group_9P542] only Reference(JP_Observation_eMunicipalCheckup_Group_9P542)
    * ^short = "検査結果セクションに含まれる 胃エックス線検査 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P542160700000011] only Reference(JP_Observation_eMunicipalCheckup_9P542160700000011)
    * ^short = "検査結果セクションに含まれる 胃がん検診の胃部エックス線検査検査判定 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P543160800000049] only Reference(JP_Observation_eMunicipalCheckup_9P543160800000049)
    * ^short = "検査結果セクションに含まれる 胃がん検診の胃部エックス線検査所見 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[Group_9P544] only Reference(JP_Observation_eMunicipalCheckup_Group_9P544)
    * ^short = "検査結果セクションに含まれる 胃内視鏡検査 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P544160700000011] only Reference(JP_Observation_eMunicipalCheckup_9P544160700000011)
    * ^short = "検査結果セクションに含まれる 胃がん検診の胃内視鏡検査検査判定 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P545160800000049] only Reference(JP_Observation_eMunicipalCheckup_9P545160800000049)
    * ^short = "検査結果セクションに含まれる 胃がん検診の胃内視鏡検査所見 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P546000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P546000000000011)
    * ^short = "検査結果セクションに含まれる 胃がん検診の精密検査の対象有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P547160800000049] only Reference(JP_Observation_eMunicipalCheckup_9P547160800000049)
    * ^short = "検査結果セクションに含まれる 胃がん検診のその他所見 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P548000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P548000000000011)
    * ^short = "検査結果セクションに含まれる 胃がん検診による偶発症の有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P549000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P549000000000011)
    * ^short = "検査結果セクションに含まれる 胃がん検診の精密検査結果 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P550160800000049] only Reference(JP_Observation_eMunicipalCheckup_9P550160800000049)
    * ^short = "検査結果セクションに含まれる 胃がん検診の精密検査所見 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P551000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P551000000000011)
    * ^short = "検査結果セクションに含まれる 胃がん検診の精密検査による偶発症の有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
...
//  * entry only Reference(JP_Observation_eMunicipalCheckup_9P540000000000011 or JP_Observation_eMunicipalCheckup_9P541000000000011)
  ...
  * entry contains
    ...
    9P541000000000011 0..1 MS
  * entry[9P540000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P540000000000011)
    * ^short = "問診結果セクションに含まれる 胃がん検診の過去の受診歴 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
  * entry[9P541000000000011] only Reference(JP_Observation_eMunicipalCheckup_9P541000000000011)
    * ^short = "問診結果セクションに含まれる 胃がん検診時の胃がんに係る症状の有無 をあらわすObservationリソースへの参照。"
    * reference 1..1
      * ^short = "参照先のリソースのfullUrl要素に指定されるUUID。"
