...
Description: """点数表コード１桁（医科１、歯科３）表現するためのExtension。 
...
* . ^comment = """点数表コード１桁（医科1、歯科３）。
Identifier型の拡張\r\n「InsuranceOrganizationCategory」を使用する。
...
* value[x].value ^comment = "If the value is a full URI, then the system SHALL be urn:ietf:rfc:3986.  The value's primary purpose is computational mapping.  As a result, it may be normalized for comparison purposes (e.g. removing non-significant whitespace, dashes, etc.)  A value formatted for human display can be conveyed using the [Rendered Value extension](extension-rendered-value.html). Identifier.value is to be treated as case sensitive unless knowledge of the Identifier.system allows the processer to be confident that non-case-sensitive processing is safe.\r\n\r\n\r\n---SWG3コメント-----\r\n点数表コード1桁（医科1、歯科3）"

Extension: JP_Organization_InsuranceOrganizationNo
Id: JP-Organization-InsuranceOrganizationNo
Description: """
 保険医療機関番号７桁を表現するためのExtension。
 JP Core Organizationプロファイルで利用されることを想定しているが、他のリソースでも利用可能である。
"""
...
* . ^short = "保険医療機関番号７桁"
* . ^comment = """
保険医療機関番号７桁。Identifier型の拡張\r\n「InsuranceOrganizationNo」を使用する。
...
// extension 参照宣言
* extension contains
    JP_Organization_PrefectureNo named PrefectureNo 0..1 and
    JP_Organization_InsuranceOrganizationCategory named OrganizationCategory 0..1 and
    JP_Organization_InsuranceOrganizationNo named OrganizationNo 0..1
//
